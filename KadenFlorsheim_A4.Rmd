  ...
  map_dfr(function(x){
    ...
    getCensus(
      ...
      mutate(
        ethnicity = "Total",
        education = case_when(
        educational_attainment %in% c("Less than 9th grade","9th to 12th grade, no diploma") ~ "Less_than_high school_diploma",
        educational_attainment %in% c("Some college, no degree","Associate's degree") ~ "Some_college_or_associates_degree",
        educational_attainment %in% c("High school graduate (includes equivalency)") ~ "High school graduate (includes equivalency)",
        educational_attainment %in% c("Bachelor's degree","Graduate or professional degree") ~ "Bachelor's degree or higher"
        )
      ) %>%
      group_by(ethnicity,education,county) %>% 
      summarize(estimate = sum(estimate)) %>% 
      mutate(year = x)
  })
```
...
bay_education_ethnicity_trend <- 
  ...
  pivot_wider(
    names_from = c(ethnicity, education),
    values_from = estimate
  ) %>% 
  mutate(
    skew = (('Less_than_high_school_diploma' + 'Some_college_or_associates_degree') / (Total) / (estimate/100))
    ) %>% 
  left_join(
    acs_vars_2019_1yr %>% 
